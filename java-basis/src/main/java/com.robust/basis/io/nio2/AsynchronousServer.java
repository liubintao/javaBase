...
public class AsynchronousServer {
    ...
    public AsynchronousServer(int port) throws IOException {
        this.port = port;
     ...
     * accept到一个请求时的回调
     */
    private class AcceptHandler implements CompletionHandler<AsynchronousSocketChannel, AsynchronousServer> {
        @Override
        public void completed(final AsynchronousSocketChannel client, AsynchronousServer attachment) {
            try {
        ...
        public void completed(Integer result, AsynchronousSocketChannel attachment) {
            try {
                if (result < 0) {// 客户端关闭了连接
                    AsynchronousServer.close(attachment);
                } else if (result == 0) {
                    System.out.println("空数据"); // 处理空数据
                } else {
                    // 读取请求，处理客户端发送的数据
                    buffer.flip();
                    CharBuffer charBuffer = AsynchronousServer.decoder.decode(buffer);
    ...
    public static void main(String[] args) {
        try {
            ...
            AsynchronousServer server = new AsynchronousServer(PORT);
            server.listen();
        } catch (Exception e) {
            e.printStackTrace();
        }
